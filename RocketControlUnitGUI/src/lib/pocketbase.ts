...
export const writeStateChange = async (state: string) => {
...
export const writeArbitraryCommand = async (target: string, command: string) => {
    await pocketbase.collection('CommandMessage').create({
        target,
        command
    });
...
export const writeLoadCellCommand = async (target: string, command: string, weight_kg: number) => {
...
export const subscribeToCollections = (timestamps: Timestamps, stores: Stores) => {
    ...
    pocketbase.collection('RcuTemperature').subscribe('*', (e) => {
        stores.rcu_tc1_temperature.set(e.record.tc1_temperature === 9999 ? 'DC' : Math.round(e.record.tc1_temperature / 100));
        stores.rcu_tc2_temperature.set(e.record.tc2_temperature === 9999 ? 'DC' : Math.round(e.record.tc2_temperature / 100));
    ...
    pocketbase.collection('DmbPressure').subscribe('*', (e) => {
        stores.upper_pv_pressure.set(e.record.upper_pv_pressure < -100000 ? 'DC' : Math.round(e.record.upper_pv_pressure / 1000));
    ...
    pocketbase.collection('PbbPressure').subscribe('*', (e) => {
        stores.ib_pressure.set(e.record.ib_pressure < -100000 ? 'DC' : Math.round(e.record.ib_pressure / 1000));
        stores.lower_pv_pressure.set(e.record.lower_pv_pressure < -100000 ? 'DC' : Math.round(e.record.lower_pv_pressure / 1000));
    ...
    pocketbase.collection('PbbTemperature').subscribe('*', (e) => {
        stores.pv_temperature.set(e.record.ib_temperature === 9999 ? 'DC' : Math.round(e.record.ib_temperature / 100));
    ...
    pocketbase.collection('SobTemperature').subscribe('*', (e) => {
        stores.sob_tc1_temperature.set(e.record.tc1_temperature === 9999 ? 'DC' : Math.round(e.record.tc1_temperature / 100));
        stores.sob_tc2_temperature.set(e.record.tc2_temperature === 9999 ? 'DC' : Math.round(e.record.tc2_temperature / 100));
